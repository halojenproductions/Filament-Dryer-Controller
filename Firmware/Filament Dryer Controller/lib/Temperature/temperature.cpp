...
OutTemp::OutTemp() : temp_() {}

OutTemp::OutTemp(int value) : temp_(adcToCelsius(value)) {}
...
int OutTemp::adcToCelsius(int adc_value) {
	const float K_C		= 273.15;	   // Conversion from Kelvin to Celsius
	const float ADC_MAX = 1023.0;	   // 10-bit ADC max value
	const float R2		= 100000.0;	   // 100kΩ pulldown resistor
	const float BETA	= 3950.0;	   // Beta value from thermistor datasheet
	const float T0		= 50 + K_C;	   // Reference temp in Kelvin (50°C)
	const float R0		= 35840.0;	   // Resistance at 50°C (35.8kΩ from your data)
