...
namespace suite_Timers {
	...
	void test_Timers_Unique(void) {
		...
		TEST_ASSERT_EQUAL(sizeof(timerIndexes) / sizeof(timerIndexes[0]), numTimers);
	...
	void test_Timer_Indexes(void) {
		...
		TEST_ASSERT_EQUAL(3, Sys::TIMER_ACTIVE_TIMEOUT);
		TEST_ASSERT_EQUAL(4, Sys::TIMER_HEATER_TIMEOUT);
		TEST_ASSERT_EQUAL(5, Sys::TIMER_HEATER_COOLDOWN);
		TEST_ASSERT_EQUAL(6, Sys::TIMER_SELECTION_TIMEOUT);
		TEST_ASSERT_EQUAL(7, Sys::TIMER_BUTTON_HOLD);
	}
}
